...
5. RETRY  — If validation fails, fix and re-validate (max 3 attempts)
```
...
After writing code, run this sequence. **All must pass:**

```bash
# 1. Tests pass with coverage
make test          # >= 80% coverage, zero failures, zero skips

# 2. Linting clean
make lint          # zero warnings

# 3. Type checking passes
# (mypy --strict / tsc --noEmit / cargo clippy / go vet)

# 4. Security audit
make audit         # no known vulnerabilities

# 5. Benchmarks compile and run (if applicable)
make bench
```
...
- **Report honestly** — if attempt 3 fails, say so with details
...
## Pre-Delivery Checklist (All Scenarios)

Before returning code, verify. **This applies to generate, fix, refactor, rewrite, add, remove, and modify.**
...
Setup:        Automated (setup.sh works)
CI:           Configured
